...
test("ignores blank and non-JSON lines", () => {
	...
	expect(parseLine("42")).toEqual([]); // valid JSON but not an object
});

test("ignores unknown and replayed-user events", () => {
	expect(parseLine(JSON.stringify({ type: "user", message: {} }))).toEqual([]);
	expect(parseLine(JSON.stringify({ type: "whatever" }))).toEqual([]);
});

test("system event yields a session id", () => {
	const out = parseLine(
		JSON.stringify({ type: "system", session_id: "abc123" }),
	);
	expect(out).toEqual([{ kind: "session", sessionId: "abc123" }]);
});

test("system event without session_id yields nothing", () => {
	expect(parseLine(JSON.stringify({ type: "system", model: "x" }))).toEqual([]);
});
...
test("result event extracts usage info", () => {
	...
	expect(ev.usage).toBeTruthy();
	expect(ev.usage?.inputTokens).toBe(100);
	expect(ev.usage?.costUSD).toBe(0.005);
});
